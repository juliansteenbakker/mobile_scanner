...
void main() {
  group('$BarcodeCapture tests', () {
    group('constructor', () {
      ...
      test('creates instance with various sizes', () {
        const sizes = [
          Size(640, 480),
          Size(1280, 720),
          Size(1920, 1080),
        ...
        for (final size in sizes) {
          final capture = BarcodeCapture(size: size);

          expect(capture.size, size);
          expect(capture.size.width, size.width);
          expect(capture.size.height, size.height);
        }
      });
